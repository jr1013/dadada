...
function renderCatalogue(filter) {
  ...
  works.forEach(w => {
    ...
    if (w.type === 'image essay')     badges.push(`<span class="fmt-badge" style="opacity:.9" onclick="event.stopPropagation(); location.href='read.html?work=${w.id}'">🎬 Cinematic read</span>`);
...
function renderDetailHeader(w) {
  ...
  document.getElementById('detHeader').innerHTML = `
    ...
    <div>
      ...
      ${w.type === 'image essay' ? `<button class="btn-preview" onclick="location.href='read.html?work=${w.id}'">🎬 Cinematic read (narrated) →</button>` : ''}
    </div>
