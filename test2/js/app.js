...
function renderDetailInfo(w) {
  ...
  document.getElementById('detInfo').innerHTML = `
    ...
    <div>
      <button class="btn-add-main" id="addBtn" onclick="addToCart()">
        ${isVideo ? 'View online — free' : 'Read online — free'}
      </button>
      ${!isVideo && w.preview ? `<button class="btn-preview" onclick="openReader('${w.id}')">Read a preview →</button>` : ''}
      ${w.type === 'photo book' ? `<button class="btn-preview" onclick="launchFlipbook('${w.id}')">📖 Open flipbook →</button>` : ''}
      ${w.type === 'image essay' ? `<button class="btn-narrator" onclick="launchNarrator('${w.id}')">▶ Narrator (auto read aloud) →</button>` : ''}
    </div>
...
/* ══════════════════════════════════════
   CHECKOUT
══════════════════════════════════════ */
function launchFlipbook(id) {
  sessionStorage.setItem('dadada_return_work', id);
  const base = location.pathname.replace('index.html', '');
  location.href = base + 'flipbook.html?work=' + id;
}

function launchNarrator(id) {
  sessionStorage.setItem('dadada_narrator_work', id);
  const base = location.pathname.replace('index.html', '');
  location.href = base + 'read.html?work=' + id;
}
